...
function applySecondaryLayoutShell(route) {
  ...
  if (!shell) {
    ...
    topbar.className = 'secondary-topbar page-header';
    topbar.innerHTML = `
...
function ensureConsolidationStyles() {
  ...
  style.textContent = `
    .conso-page{--conso-space-1:4px;--conso-space-2:8px;--conso-space-3:12px;--conso-space-4:16px;--conso-card-pad:10px;padding:var(--conso-space-2) var(--conso-space-2) calc(env(safe-area-inset-bottom,0px) + 14px);color:#eaf2ff;background:linear-gradient(180deg,#132760 0%,#050b28 100%);border-radius:22px;position:relative;overflow:hidden;}
    ...
    .conso-card{background:linear-gradient(180deg,rgba(18,32,70,.75),rgba(10,18,42,.78));border:1px solid rgba(160,190,255,.2);border-radius:22px;box-shadow:0 14px 34px rgba(0,0,0,.4);padding:var(--conso-card-pad);margin-bottom:8px;}
    .conso-hero{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.7fr);gap:8px;align-items:center;min-height:106px}.conso-hero img{width:100%;max-width:124px;max-height:82px;object-fit:contain;justify-self:center}.conso-hero h2{font-size:21px;line-height:1.08;margin:0 0 3px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.conso-hero p{margin:0;color:#ceddfa;font-size:12px;line-height:1.28;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
    .conso-stats,.stats-row{margin-top:8px;background:rgba(12,23,56,.68);border:1px solid rgba(144,178,241,.24);border-radius:14px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0}
    .conso-stats article{padding:6px;display:grid;gap:1px;align-content:start;min-height:62px}.conso-stats article + article{border-left:1px solid rgba(144,178,241,.2)}
    .conso-stats b{font-size:10px;color:#dce8ff;display:flex;align-items:center;gap:3px;min-height:20px}.conso-stats strong{font-size:17px;line-height:1.05}.conso-stats small{font-size:10px;color:#dce8ff}
    .conso-zone{display:flex;gap:6px;align-items:center;margin-top:6px;justify-content:flex-end}.conso-zone span{font-size:11px;color:#ccdcfa;white-space:nowrap}.conso-zone input{width:88px;height:36px;padding:8px 10px}
    .conso-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .conso-tile{background:linear-gradient(180deg,rgba(15,29,68,.84),rgba(11,19,46,.9));border:1px solid rgba(143,177,241,.28);border-radius:18px;padding:10px;min-height:136px;display:grid;grid-template-rows:auto 1fr;gap:6px;text-align:left;width:100%;transition:transform .14s ease, box-shadow .14s ease;box-shadow:0 10px 22px rgba(4,8,24,.45)}
    .conso-tile:active{transform:scale(.985);box-shadow:0 16px 28px rgba(69,136,255,.3)}.conso-tile-icon-wrap{display:flex;justify-content:center;align-items:flex-start;height:62px;padding-top:2px}.conso-tile img{width:84%;max-width:112px;height:58px;object-fit:contain;display:block}
    .conso-tile span{display:grid;align-content:start;gap:2px;padding:0;background:none;color:#f1f6ff}.conso-tile strong{font-size:15px;line-height:1.18}.conso-tile small{font-size:12px;color:#d9e6ff}
    .conso-ai{padding:9px 10px}.conso-ai header{display:flex;align-items:center;justify-content:space-between;gap:8px}.conso-ai-title{display:flex;align-items:center;gap:8px;font-size:16px;font-weight:700}
    .conso-pill{border-radius:999px;padding:6px 10px;min-height:36px;background:rgba(73,96,159,.35);border:1px solid rgba(146,177,237,.35);color:#e6efff;font-size:12px}
    .conso-input-row{display:flex;gap:8px;margin-top:8px;align-items:center}.conso-input-row input,.conso-input{background:rgba(16,30,67,.82);border:1px solid rgba(146,178,240,.34);border-radius:16px;color:#e9f1ff;padding:10px 12px;width:100%;min-height:44px}
    .conso-send{width:44px;height:44px;min-width:44px;border-radius:50%;border:1px solid rgba(146,177,238,.38);background:linear-gradient(180deg,#55a6ff,#2c5bc8);color:white}
    .conso-ai-output article{margin-top:8px;background:rgba(8,14,38,.6);padding:10px;border-radius:12px}
    .conso-page h3{margin:4px 0 10px}.conso-list{display:flex;flex-direction:column;gap:8px}
    ...
    .conso-row button,.conso-move button,.conso-btn{background:rgba(77,106,176,.35);color:#d8e6ff;border:1px solid rgba(146,177,237,.35);border-radius:12px;padding:6px 10px;min-height:44px}
    ...
    @media (max-height:760px){.conso-page{--conso-card-pad:8px;padding:6px 8px calc(env(safe-area-inset-bottom,0px) + 12px)}.conso-hero{min-height:92px}.conso-hero h2{font-size:19px}.conso-hero p{font-size:11px;-webkit-line-clamp:2}.conso-stats strong{font-size:15px}.conso-tile{min-height:124px;padding:8px}.conso-tile-icon-wrap{height:54px}.conso-tile img{height:50px}}
...
function ensureRemiseStyles() {
  ...
  style.textContent = `
    .remise-page{padding:calc(env(safe-area-inset-top,0px) + 8px) 10px calc(env(safe-area-inset-bottom,0px) + 14px);color:#ebf3ff;background:linear-gradient(180deg,#12285f 0%,#040a28 100%);border-radius:26px;position:relative;overflow:hidden}
    ...
    .rem-card{padding:11px;margin-bottom:9px}.rem-hero{display:grid;grid-template-columns:1.15fr .85fr;align-items:center;gap:10px}.rem-hero h2{font-size:34px;line-height:1.05;margin:0 0 6px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.rem-hero p{font-size:14px;margin:0 0 6px;color:#c7d7f6;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.rem-hero small{font-size:12px;color:#b6c8eb}.rem-hero img{width:100%;max-height:108px;object-fit:cover;border-radius:16px}
    .rem-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:9px}.rem-tile{padding:0;overflow:hidden;text-align:left;transition:transform .14s ease, box-shadow .14s ease;min-height:140px;width:100%}.rem-tile:active{transform:scale(.985);box-shadow:0 18px 38px rgba(59,126,255,.45)}.tile-icon-wrap{height:72px;display:flex;align-items:flex-start;justify-content:center;padding:8px 8px 2px}.rem-main img{width:72px;height:72px;object-fit:cover;display:block;border-radius:14px;border:1px solid rgba(160,190,255,.24)}
    .rem-main span{display:flex;flex-direction:column;padding:10px;background:rgba(11,20,48,.88);gap:2px}.rem-main strong{font-size:16px;line-height:1.2}.rem-main small{font-size:13px;color:#d4e3ff}
    .rem-short{display:flex;align-items:center;min-height:64px;padding:8px 12px}.rem-short strong{font-size:16px;line-height:1.2}.rem-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
    .rem-ai header{display:flex;align-items:center;justify-content:space-between;gap:8px}.rem-ai h3{margin:0;display:flex;align-items:center;gap:8px;font-size:18px}.rem-pill,.rem-btn{border-radius:999px;padding:7px 11px;min-height:36px;border:1px solid rgba(146,177,237,.34);background:rgba(67,96,160,.3);color:#e3eeff}
    .rem-ai-form{display:flex;gap:8px;margin-top:8px;align-items:center}.rem-ai-form input,.rem-input{flex:1;background:rgba(16,30,67,.82);border:1px solid rgba(146,178,240,.34);border-radius:16px;color:#e9f1ff;padding:10px 12px;min-height:44px}
    .rem-send{width:44px;height:44px;border-radius:50%;border:1px solid rgba(146,177,238,.38);background:linear-gradient(180deg,#67b0ff,#2f5fd0);color:#fff}
    ...
    .rem-mini-nav{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.rem-mini{padding:8px 6px;display:flex;flex-direction:column;align-items:center;gap:3px;min-height:44px}.rem-mini b{font-size:13px}
    .rem-mini.is-active{box-shadow:0 -3px 0 #46a1ff inset,0 0 0 1px rgba(96,166,255,.5)}.rem-mini span{font-size:18px}
    ...
    @media (max-width:760px){.rem-hero h2{font-size:27px}.rem-hero{grid-template-columns:1fr}.tile-icon-wrap{height:66px}.rem-main img{width:68px;height:68px}.rem-short strong{font-size:15px}}
  `;
  document.head.appendChild(style);
}
